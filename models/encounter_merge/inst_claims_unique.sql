...
with encounter_stage as(
  select
    h.bene_mbi_id || h.clm_type_cd || isnull(h.prncpl_dgns_cd,'')  || isnull(h.fac_prvdr_npi_num,'')  || isnull(h.atndg_prvdr_npi_num,'') || isnull(r.clm_line_prod_rev_ctr_cd,'')  || replace(cast(h.clm_thru_dt as date),'-','') || replace(cast(ISNULL(h.clm_from_dt,'1900-01-01') as date),'-','')
     as encounter_id
    ,h.cur_clm_uniq_id as claim_id
    ,h.clm_query_cd as claim_query_code
    ,h.clm_bill_freq_cd as claim_frequency_code
...
select
	...
	,row_number() over (partition by s.encounter_id order by 
  														case when claim_query_code = 5 then 1   -- Final Bill first priority
  															 when claim_query_code = 3 then 2   -- Final Bill first priority
                                                             when claim_query_code = 3 and claim_frequency_code = 9 then 3   -- Final bill and admit/discharge thru claim
                                                             when claim_query_code = 3 and claim_frequency_code = 7 then 4   -- 
                                                             when claim_query_code = 3 and claim_frequency_code = 4 then 5   -- 
                                                        	     else 3 end)
	as row_number
...
inner join encounter_duplicates d
	on h.CUR_CLM_UNIQ_ID = d.claim_id
	and d.row_number = 1
inner join {{ref('inst_payment_adjustment')}} a
	on a.CUR_CLM_UNIQ_ID = d.claim_id
