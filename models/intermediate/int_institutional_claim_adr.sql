...
/* 
    ...
    removing "#" from claim id before it's used in partition 
*/
, dedupe as (
...
, sort_adjusted_claims as (
    ...
    select
        ...
        , clm_bill_clsfctn_cd
        , case
            when clm_adjsmt_type_cd = '1' then cast(clm_pmt_amt as {{dbt.type_numeric()}})  * -1
            else cast(clm_pmt_amt as {{dbt.type_numeric()}} )
          end as clm_pmt_amt
        , bene_ptnt_stus_cd
        , dgns_drg_cd
        , fac_prvdr_npi_num
        ...
        , dgns_prcdr_icd_ind
        , case
            when clm_adjsmt_type_cd = '1' then cast(clm_mdcr_instnl_tot_chrg_amt as {{dbt.type_numeric()}})  * -1
            else cast(clm_mdcr_instnl_tot_chrg_amt as {{dbt.type_numeric()}}) 
